...
con_str = urllib.parse.quote_plus(
    "DRIVER={ODBC Driver 17 for SQL Server};SERVER=interlake-bi.database.windows.net;DATABASE=ISS_DW;UID=BIAdmin;PWD=sb98D&B(*#$@")
...
base_url_part1 = 'https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?begin_date=' + \
    previous_day_str + '&end_date=' + next_day_str + '&station='
...
def main(mytimer: func.TimerRequest) -> None:
    final_station_list = [base_url_part1 +
                          str(i) + base_url_part2 for i in stations_lst]
    ...
    for url in final_station_list:
        ...
        for d in data['data']:
            ...
            time = d['t']
            waterlevel = d['v']
            rows.append({"StationId": stationid, "Time": time,
    ...
    final_df = final_df.astype(
        {'WaterLevel': 'float64', 'Time': 'datetime64[ns]', 'StationId': 'int64'})
    ...
    df_ff.to_sql('WaterLevelMeasurements', con=engine,
                 index=False, if_exists='append', schema='dbo')
