...
stations_lst = [9063090,9044020,9044030,9044036,9044049,9034052,9014070,9014090,9014098,9075099,9076024,9076027,9076033,9076060,9076070,9099004,9075080,9087031,9087044,9087072,9087057,9099064,9099018,9063079,9063053,9063063]
...
def ingest_from_noaa():
     ...
     final_df['WaterLevel'] = final_df['WaterLevel'].apply(lambda x: (x * 10))

     # df_ff = pd.merge_asof(
     #       stations_final_df.sort_values(['Time']),
     ...
     #df_ff.to_sql('WaterLevelMeasurements_dev2', con=engine, index=False, if_exists='append', schema='dbo')
